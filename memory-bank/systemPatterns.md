...
- **Automatic index creation**: 4 performance indexes created automatically on database open via `open_db()` → `ensure_indexes()`:
...
- **Uses standard conventional commits**:
  - `fix:` commits trigger patch releases (0.12.0 → 0.12.1)
  - `feat:` commits trigger minor releases (0.12.0 → 0.13.0)
...
- **Template storage**: TOML files in `~/.sdbh/templates/` directory with automatic creation
- **Variable substitution**: `{variable_name}` syntax with regex-based parsing and replacement
- **Template validation**: Comprehensive validation of template structure, variable definitions, and command syntax
- **Interactive prompting**: Smart prompting for missing required variables with default value support
- **Template management**: Full CRUD operations (create, read, update, delete) with file-based persistence
- **Category support**: Optional template categorization for organization and filtering
- **Error handling**: Robust error handling for file operations, invalid TOML, missing variables
- **CLI integration**: Complete `sdbh template` command suite with all management operations
